...
```sh
pip install -e ./examples/audius/audius-source          # into the same env as quickbeam
...
quickbeam data audius --dry-run --side all --cache-file ./examples/audius/audius-build/audius_cache.json

# 2. Stage both graphs FROM THAT CACHE (no network, seconds).
quickbeam data audius --side A --cache-file ./examples/audius/audius-build/audius_cache.json \
  --output-dir ./examples/audius/audius-build/stage --volume 1
quickbeam data audius --side B --cache-file ./examples/audius/audius-build/audius_cache.json \
  --output-dir ./examples/audius/audius-build/stage --volume 2

# 3. Embed both into one collection, badged by publisher.
quickbeam data prebake --input-dir ./examples/audius/audius-build/stage --volume 1 \
  --collection audius --dim 256 --owner 0xPLATFORM…
quickbeam data prebake --input-dir ./examples/audius/audius-build/stage --volume 2 \
  --collection audius --dim 256 --owner 0xARTIST…

# 4. The linkset, then bake + install it.
audius-link --cache-file ./examples/audius/audius-build/audius_cache.json --out ./examples/audius/audius-build/linkset.json
quickbeam cdn bake --config ./examples/audius/audius-build/domains.audius.json --domain audius \
  --collection audius --cdn-dir ./examples/audius/audius-build/cdn
quickbeam cdn edges --cdn-dir ./examples/audius/audius-build/cdn --domain audius \
  --source ./examples/audius/audius-build/linkset.json
quickbeam cdn serve --cdn-dir ./examples/audius/audius-build/cdn --cors
```
...
```sh
python -m pytest examples/audius/audius-source/tests/ -q
```
