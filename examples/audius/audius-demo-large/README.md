...
```sh
cd ../../..                              # repo root (quickbeam/)
docker start qdrant                      # 1.9M embedded records

venv/bin/python -m quickbeam.cli cdn serve \
    --cdn-dir ./examples/audius/audius-large-build/cdn --cors --port 8092

venv/bin/python -m quickbeam.cli serve \
    --collection audius-large --port 8081 \
    --index-layout ./examples/audius/audius-large-build/cdn/audius-large/index/layout.json \
    --adjacency-db ./examples/audius/audius-large-build/stage/edges.sqlite

cd examples/audius/audius-demo-large && npm run dev       # http://localhost:5181
```
