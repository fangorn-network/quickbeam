      ...
      <main className="main">
        {err && (
          <div className="empty">
            ...
            <p>{err}</p>
            {/* These are THIS build's paths and ports. They were inherited from
                audius-demo and pointed at ./examples/audius/audius-build/cdn on :8090, which sends
                anyone hitting this panel to start the wrong server on the wrong
                directory — the one error message someone reads only when already
                stuck. */}
            <p style={{ marginTop: 12 }}>
              Start the snapshot server with
              {' '}<code>quickbeam cdn serve --cdn-dir ./examples/audius/audius-large-build/cdn --cors --port 8092</code>.
            </p>
            {/* A 404 on a shard is almost never a missing server — it is a stale
                manifest. Shard filenames carry a content hash, so a rebuild mints new
                names and a browser holding the old manifest asks for files that no
                longer exist. Naming that here saves the next person the hunt. */}
            <p style={{ marginTop: 8 }}>
              A <code>shard 404</code> usually means a <b>stale manifest</b>: shard
              filenames contain a content hash, so rebuilding the snapshot renames them
              all. Hard-reload the page (<code>Ctrl/Cmd&nbsp;+&nbsp;Shift&nbsp;+&nbsp;R</code>)
              to pick up the new manifest.
            </p>
            <p style={{ marginTop: 8, color: 'var(--text-faint)', fontSize: 13 }}>
              Viewing over a tunnel? Tunnel the app port only (<code>5181</code>) — the
              snapshot is proxied at <code>/cdn</code> and the search API at
              {' '}<code>/api</code> on this same origin. Pointing the browser at
              {' '}<code>localhost:8092</code> makes it look for the server on
              <em> your</em> device.
            </p>
          </div>
        )}
        ...
        {!stats && !err && (
          <div className="loading">
            ...
            <span style={{ fontSize: 13, color: 'var(--text-faint)', maxWidth: '46ch', textAlign: 'center' }}>
              A starting slice of the catalogue is coming to you, once. Searching
              reaches the rest — your query is matched here and only a bucket number
              is ever sent, never what you typed.
            </span>
          </div>
        )}
