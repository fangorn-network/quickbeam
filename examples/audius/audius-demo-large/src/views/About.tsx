...
export default function About({ stats }: { stats: Stats }) {
  ...
  // Catalogue figures where the text describes what each publisher HOLDS; resident
  // figures only where it describes what was downloaded. Conflating them either
  // understates the catalogue 57x or overstates what is in the tab.
  const cat = (p?: typeof a) => (p && stats.catalogue?.publishers.find(
  ...
  return (
    <section className="about">
      ...
      <dl className="about-defs">
        ...
        <dd>
          The public catalogue — {(A?.counts.Track ?? 0).toLocaleString()} tracks
      ...
      <p>
        ...
        fills it. {(stats.catalogue?.records ?? stats.records).toLocaleString()} records
        in total, and nothing had to be merged into one database to make that work.
      </p>
      ...
      <p>
        A {stats.records.toLocaleString()}-record starting slice is downloaded once,
        up front, and everything on this first screen is served from it. Searching
        reaches the other{' '}
        {((stats.catalogue?.records ?? stats.records) - stats.records).toLocaleString()}{' '}
        records without sending what you typed: the query is embedded and matched
        against a public codebook in this tab, and only a bucket number is requested.
        The embedding model, the similarity scan and the session kernel all run in a
        Web Worker, off the main thread — <em>knowledge is public, intent is
        private</em>.
