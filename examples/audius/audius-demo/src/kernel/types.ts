// `import type` (not a bare import) so node's type-stripping can erase it — `Vec`
// is a type alias, and a value-shaped import of it fails at runtime under
// --experimental-strip-types, which is how the node-side checks run this.
...
/**
 * One candidate to be reweighted.
 *
 * Named `ChromaHit` in sonder, where candidates came back from ChromaDB. There is
 * no Chroma here — candidates come from the in-worker vector scan — so that name
 * would mislead, but the SHAPE is unchanged so the two stay diffable.
 *
 * `distance` is **squared L2**, which is what `reweight`'s Gaussian expects. Our
 * search produces cosine, so it must go through `cosToDistance` in adapt.ts.
 */
