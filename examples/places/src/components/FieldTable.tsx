...
import { formatDuration, formatRating, splitList, parseJsonArray } from '../lib/labels';
...
interface Props {
  ...
  onShowJson: () => void;
  hideFields?: string[]; // keys promoted elsewhere (header/hours) — not repeated here
}
...
export default function FieldTable({ fields, onSoftLink, onShowJson, hideFields }: Props) {
  const domain = useDomain();
  const [showAll, setShowAll] = useState(false);
  const hidden = new Set(hideFields ?? []);

  // Keep scalar fields + tag arrays (rendered as pills). Other array fields are the
  // "Connections" and render elsewhere. Suppressed/promoted fields never show here.
  const entries = Object.entries(fields).filter(([k, v]) => {
    if (domain.isSuppressedField(k) || hidden.has(k)) return false;
...
function renderValue(
  ...
  // Scalar strings that hold a JSON-encoded array → pills.
  const jsonArr = parseJsonArray(v);
  if (jsonArr) {
    return (
      <span className={styles.pills}>
        {jsonArr.map((t) => (
