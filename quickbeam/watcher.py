...
async def _stream_source_once(args, qdrant, embed_engine, role_map_ref, dim, truncate,
    ...
    """Seed one owner:namespace source, then consume its `fangorn subscribe` stream
    until it ends. Returns normally when the stream closes (caller resubscribes)."""
    key = f"{owner}:{namespace}"
    ...
    try:
        # Seed current on-chain state so the existing corpus is embedded before we go
        # live. The in-memory snapshot below is grown by each streamed diff.
        try:
            contents = read_source(args.fangorn_bin, owner, namespace)
        except Exception as e:  # noqa: BLE001
            print(f"[Watcher] {key}: seed read failed ({e}); starting from empty state",
                  file=sys.stderr)
            contents = {"vertices": [], "edges": []}

        vertices_by_cid = {v["cid"]: v for v in contents.get("vertices", [])}
        edges_by_key = {_edge_key(e): e for e in contents.get("edges", [])}
