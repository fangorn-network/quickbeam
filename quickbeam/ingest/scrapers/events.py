"""
EventsSource — events_raw (Eventbrite/Tribe payloads) → Fangorn graph, merged into
the local-business (places) graph. The harness `Source` port of `pipelines/events_pg`.

A BATCH source: there is no live tail, so `next_cursor` never advances and the
harness's checkpoint/watch/accumulate machinery no-ops (all stems are snapshots).
The value the harness adds here is the shared CLI, `emit_volumes`, and `--publish`.

  Nodes : Event, Organizer  (+ reuses Category, Locality from the places graph)
  Edges : hostedBy (Event→Organizer), inCategory (Event→Category),
          locatedIn (Event→Locality), hostedAt (Event→Business, the merge link by
          coordinate/name match), hostsEvent (Business→Event, the reverse)

Default output volume is 2, so it coexists with the places `volume_1_*` files. The
merge link resolves each Event's venue to the nearest existing `Business` (loaded
from `volume_1_businesses.json`) within `--match-radius-m`.
...
# THE SOURCE
# ===========================================================================
class EventsSource(SourceBase):
    name = "events"
    default_volume = 2
    stems = {"Event": "events", "Organizer": "organizers",
             "Category": "event_categories", "Locality": "event_localities"}
    # Batch source: re-shaped wholesale each run, so all stems are snapshots
    # (—accumulate then still replaces rather than ledgering stale events).
    snapshot_stems = set(stems.values())
    role_map = {"title": "title", "subtitle": "venueName",
                "tags": ["source", "priceLevel"], "text": ["text"]}
    presentation = {"accent": "#d0021b", "icons": {"Event": "event",
                    "Organizer": "groups", "Category": "sell", "Locality": "place"}}

    def add_source_args(self, p: argparse.ArgumentParser) -> None:
        p.add_argument("--dsn", default=DEFAULT_DSN,
                       help="Postgres connection (or env EVENTS_PG_DSN).")
        p.add_argument("--raw-in", default="",
                       help="Read raw rows from this JSONL file instead of Postgres.")
    ...
    def read(self, cursor: int, args: argparse.Namespace) -> list[dict]:
        # Side inputs consumed by build_graph — stashed on the instance (they're
        # config for the shaping, not raw records).
        biz_path = args.businesses_in or os.path.join(args.output_dir,
                                                       "volume_1_businesses.json")
        self._businesses = load_businesses(biz_path)
        self._radius = args.match_radius_m
        if self._businesses:
            print(f"🔗 Business index: {len(self._businesses)} from "
                  f"{os.path.basename(biz_path)}")
        else:
        ...
        # Normalize each raw row into one common event dict (materialize the stream).
        out = []
        for row in rows:
            norm = (normalize_eventbrite if row["source"] == "eventbrite"
                    else normalize_tribe)(row["payload"])
            out.append(norm)
        return out

    def build_graph(self, records: list[dict]) -> tuple[dict[str, list[dict]], list[dict]]:
        """Common events → typed nodes + edges. Pure w.r.t. `records`, but closes over
        the business index / radius stashed on the instance by `read`. Mirrors the old
        `events_pg.run_export` loop exactly (row order; first-seen dedup)."""
        businesses = getattr(self, "_businesses", [])
        radius_m = getattr(self, "_radius", 120.0)
        events: list[dict] = []
        ...
        for ev in records:
            ...
            events.append({"name": ekey, "fields": fields})

            okey = _org_key(ev)
            if okey:
                if okey not in seen_org:
                    seen_org.add(okey)
                    organizers.append({"name": okey, "fields": shape_organizer(ev)})
            ...
            for cname in ev.get("categories") or []:
                ...
                if cslug not in seen_cat:
                    seen_cat.add(cslug)
                    categories.append({"name": cslug, "fields": shape_category(cname)})
        ...
        # Always emit all four node stems (empty lists → empty files), matching the
        # old run_export which opened a writer per type.
        nodes = {"Event": events, "Organizer": organizers,
