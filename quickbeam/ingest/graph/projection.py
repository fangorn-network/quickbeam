...
def _load_profiles(args) -> list[dict]:
    """Resolve --root-profile names into a list of fully-specified profile dicts.
    Each carries a `name` and `root_type`.
    """
    ...
    if not args.root_profile:
        raise SystemExit(
            "No --root-profile given. Pass at least one, e.g. --root-profile asset "
            f"(known: {', '.join(sorted(registry))}, or define one in --profiles-file).")
