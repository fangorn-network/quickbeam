  ...
  try {
    ...
    switch (op) {
      ...
      case 'search':
        ...
        break;
      // The *Async variants try the resident graph first and fall back to the hosted
      // API. That fallback is what makes a search result CLICKABLE: without it a hit
      // fetched from a bucket opens to "Not in this snapshot" (Entity.tsx) and its
      // page shows no rails, even though the graph holds thousands of edges for it.
      case 'entity':    result = await graph.entityAsync(args.id as string); break;
      case 'relations': result = await graph.relationsAsync(args.id as string); break;
      case 'neighbours':
        result = await graph.neighboursAsync(args.id as string, args.rel as string,
                                             args.dir as 'out' | 'in',
